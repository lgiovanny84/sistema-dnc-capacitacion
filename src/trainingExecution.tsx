...
export function TrainingExecutionModule({
  ...
  return (
    <>
      ...
      {section === "report" && <ExecutionReport records={records} batches={batches} period={period} reload={reload} />}
    </>
  );
}
...
function ExcelImporter({ records, batches, period, reload }: { records: TrainingRecord[]; batches: ImportBatch[]; period: { id: string; name: string; start_date: string; end_date: string }; reload: () => Promise<void> }) {
  ...
  return <div className="panel">
    ...
    {rows.length > 0 && <><CollapsibleTable className="preview"><table><thead><tr>{TRAINING_HEADERS.slice(0, 8).map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{rows.slice(0, 8).map((row) => <tr key={row.source_row_hash}><td>{row.name}</td><td>{row.gender}</td><td>{row.position}</td><td>{row.requesting_area}</td><td>{row.office}</td><td>{row.topic}</td><td>{row.company}</td><td>{row.start_date || "—"}</td></tr>)}</tbody></table></CollapsibleTable><div className="actions"><button className="primary" disabled={busy} onClick={() => void upload()}>{busy ? "Cargando…" : `Cargar ${rows.filter((r) => !known.has(r.source_row_hash)).length} filas nuevas`}</button></div></>}
    <div className="batch-manager"><div className="panelhead"><div><h3>Bases cargadas en {period.name}</h3><p>Eliminar una base borra únicamente los registros asociados a ese lote.</p></div><button className="secondary danger" disabled={!records.length} onClick={() => void resetRecords()}>Resetear bases del período</button></div><CollapsibleTable><table><thead><tr><th>Período</th><th>Desde</th><th>Hasta</th><th>Archivo</th><th>Filas</th><th>Fecha de carga</th><th>Acción</th></tr></thead><tbody>{batches.map((batch) => <tr key={batch.id}><td><b>{batch.period_name}</b></td><td>{batch.period_start}</td><td>{batch.period_end}</td><td>{batch.source_filename}</td><td>{batch.row_count}</td><td>{new Date(batch.uploaded_at).toLocaleString("es-EC")}</td><td><button className="secondary danger" onClick={() => void deleteBatch(batch)}>Eliminar base</button></td></tr>)}{!batches.length && <tr><td colSpan={7}>No existen bases cargadas en este período.</td></tr>}</tbody></table></CollapsibleTable></div>
  </div>;
}
...
function ExecutionRecordEditor({ record, period, onCancel, onSaved }: { record: TrainingRecord; period: { name: string; start_date: string; end_date: string }; onCancel: () => void; onSaved: () => Promise<void> }) {
  const [draft, setDraft] = useState(record);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState("");
  const textFields: Array<[keyof TrainingRecord, string]> = [
    ["name", "Nombre"], ["gender", "Género"], ["position", "Cargo"], ["requesting_area", "Área requiriente"], ["office", "Oficina"], ["topic", "Tema"], ["company", "Empresa"], ["location", "Lugar"], ["status", "Estado"], ["justification_criterion", "Criterio de justificación"], ["modality", "Modalidad"], ["area", "Área"], ["department", "Departamento"], ["level", "Nivel"], ["occupational_group", "Grupo ocupacional"], ["knowledge_area", "Área de conocimiento"],
  ];
  const set = (field: keyof TrainingRecord, value: string | number | null) => setDraft((current) => ({ ...current, [field]: value }));
  async function save(e: FormEvent) {
    e.preventDefault();
    if (draft.start_date && draft.end_date && draft.end_date < draft.start_date) { setMessage("La fecha de fin no puede ser anterior a la fecha de inicio."); return; }
    if ((draft.start_date && (draft.start_date < period.start_date || draft.start_date > period.end_date)) || (draft.end_date && (draft.end_date < period.start_date || draft.end_date > period.end_date))) { setMessage(`Las fechas deben estar dentro del período ${period.name}.`); return; }
    setSaving(true);
    const editable = Object.fromEntries([...textFields.map(([field]) => [field, draft[field]]), ["start_date", draft.start_date || null], ["end_date", draft.end_date || null], ["duration", Number(draft.duration)], ["cost", Number(draft.cost)]]);
    const { error } = await supabase!.from("training_records").update(editable).eq("id", record.id);
    setSaving(false);
    setMessage(error?.message ?? "Registro de capacitación actualizado.");
    if (!error) await onSaved();
  }
  return <form className="panel form execution-record-editor" onSubmit={save}><div className="panelhead"><div><h2>Editar capacitación recibida</h2><p>Los cambios quedan registrados en la bitácora institucional.</p></div></div><div className="fields">{textFields.map(([field, label]) => <label key={field}>{label}<input value={String(draft[field] ?? "")} onChange={(e) => set(field, e.target.value)} required={["name", "requesting_area", "topic"].includes(field)} /></label>)}<label>Fecha de inicio<input type="date" min={period.start_date} max={period.end_date} value={draft.start_date ?? ""} onChange={(e) => set("start_date", e.target.value || null)} /></label><label>Fecha de fin<input type="date" min={draft.start_date || period.start_date} max={period.end_date} value={draft.end_date ?? ""} onChange={(e) => set("end_date", e.target.value || null)} /></label><label>Duración<input type="number" min="0" step="0.01" value={draft.duration} onChange={(e) => set("duration", Number(e.target.value))} /></label><label>Costo<input type="number" min="0" step="0.01" value={draft.cost} onChange={(e) => set("cost", Number(e.target.value))} /></label></div>{message && <div className="statusmsg">{message}</div>}<div className="actions"><button type="button" className="secondary" onClick={onCancel}>Cancelar</button><button className="primary" disabled={saving}>{saving ? "Guardando…" : "Guardar cambios"}</button></div></form>;
}

function ExecutionReport({ records, batches, period, reload }: { records: TrainingRecord[]; batches: ImportBatch[]; period: { name: string; start_date: string; end_date: string }; reload: () => Promise<void> }) {
  ...
  return <>{editing && <ExecutionRecordEditor record={editing} period={period} onCancel={() => setEditing(null)} onSaved={async () => { await reload(); setEditing(null); }} />}<div className="panel filters execution-filters"><label>Base / período<select value={filters.batch} onChange={(e) => set("batch", e.target.value)}><option value="">Todas</option>{batches.map((batch) => <option key={batch.id} value={batch.id}>{batch.period_name} — {batch.source_filename}</option>)}</select></label><label>Año<input type="number" min="2020" max="2100" value={filters.year} onChange={(e) => set("year", e.target.value)} /></label><label>Desde<input type="date" min={period.start_date} max={period.end_date} value={filters.from} onChange={(e) => set("from", e.target.value)} /></label><label>Hasta<input type="date" min={period.start_date} max={period.end_date} value={filters.to} onChange={(e) => set("to", e.target.value)} /></label>{(["gender", "requesting_area", "office", "status", "modality", "department", "occupational_group", "knowledge_area"] as const).map((field) => <label key={field}>{field === "requesting_area" ? "Área requiriente" : field === "occupational_group" ? "Grupo ocupacional" : field === "knowledge_area" ? "Área de conocimiento" : field.charAt(0).toUpperCase() + field.slice(1)}<select value={filters[field]} onChange={(e) => set(field, e.target.value)}><option value="">Todos</option>{values(field).map((value) => <option key={value}>{value}</option>)}</select></label>)}<button className="secondary" onClick={() => setFilters({ batch: "", year: "", from: "", to: "", gender: "", requesting_area: "", office: "", status: "", modality: "", department: "", occupational_group: "", knowledge_area: "" })}>Limpiar</button></div>
    ...
    <div className="panel"><div className="panelhead"><div><h2>Reporte detallado</h2><p>{filtered.length} registros con las 20 columnas de la base fuente. El administrador puede corregir cada registro.</p></div><div><button className="secondary" onClick={() => window.print()}>Imprimir / PDF</button> <button className="primary" onClick={() => void exportExcel()}>Exportar Excel</button></div></div><CollapsibleTable className="execution-table"><table><thead><tr>{TRAINING_HEADERS.map((h) => <th key={h}>{h}</th>)}<th>Acción</th></tr></thead><tbody>{filtered.map((r) => <tr key={r.id}><td>{r.name}</td><td>{r.gender}</td><td>{r.position}</td><td>{r.requesting_area}</td><td>{r.office}</td><td>{r.topic}</td><td>{r.company}</td><td>{r.start_date}</td><td>{r.end_date}</td><td>{r.duration}</td><td>{r.location}</td><td>{money(r.cost)}</td><td>{r.status}</td><td>{r.justification_criterion}</td><td>{r.modality}</td><td>{r.area}</td><td>{r.department}</td><td>{r.level}</td><td>{r.occupational_group}</td><td>{r.knowledge_area}</td><td><button className="secondary" onClick={() => setEditing(r)}>Editar</button></td></tr>)}</tbody></table></CollapsibleTable></div></>;
}
