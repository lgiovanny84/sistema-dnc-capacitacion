...
export function CompetencyMatrixManager({ rows, reload }: { rows: CompetencyMatrixRow[]; reload: () => Promise<void> }) {
  ...
  return <section className="panel competency-manager">
    ...
    <CollapsibleTable className="matrix-table"><table><thead><tr><th>Área</th><th>Departamento</th><th>Grupo</th><th>Factor</th><th>Competencia</th><th>Descripción</th><th>Estado</th></tr></thead><tbody>{visible.map((row) => { const value = draft(row); return <tr key={row.id}><td><input value={value.area} onChange={(e) => change(row, { area: e.target.value })} /></td><td><input value={value.department} onChange={(e) => change(row, { department: e.target.value })} /></td><td><input value={value.group_name} onChange={(e) => change(row, { group_name: e.target.value })} /></td><td><input value={value.factor} onChange={(e) => change(row, { factor: e.target.value })} /></td><td><input value={value.competency} onChange={(e) => change(row, { competency: e.target.value })} /></td><td><input value={value.description} onChange={(e) => change(row, { description: e.target.value })} /></td><td><button className={value.active ? "tag" : "tag off"} onClick={() => change(row, { active: !value.active })}>{value.active ? "Activo" : "Inactivo"}</button></td></tr>; })}</tbody></table></CollapsibleTable>
  </section>;
}
